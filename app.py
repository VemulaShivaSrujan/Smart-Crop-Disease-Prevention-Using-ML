...
def crop_prediction():
    ...
    if request.method == 'POST':
        ...
        crops = crop_recommendation_model.classes_
        top2_crops = [crops[i] for i in top2_indices]

        # ✅ Soil-based filtering (improves realism)
        if soil == "sandy":
            top2_crops = [crop for crop in top2_crops if crop not in ["rice", "jute"]] or top2_crops
        elif soil == "clay":
            if "rice" not in top2_crops:
                top2_crops.append("rice")
        elif soil == "loamy":
            top2_crops.append("wheat")

        # Keep only 2 unique crops
        top2_crops = list(dict.fromkeys(top2_crops))[:2]
